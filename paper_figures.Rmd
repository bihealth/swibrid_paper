...
```{r FigS1X, fig.width=1.75 * fig_scale, fig.height=1.5 * fig_scale}
                          ...
                          function(donor) 
                            lapply(if (donor=='mixed') seq(1,5) else seq(1,3),
                                   function(rep)
                                     ...
                                     {
                                       fname <- paste0('../human_samples/20240307_hg38_results/cellswitch_combined/',
...
ggplot(meta_clustering %>%
         ...
         dplyr::ungroup() %>% 
         dplyr::distinct(Donor, ncells, frac_clusters) %>%
         ...
         # dplyr::group_by(Donor, ncells) %>%
         # dplyr::summarise(mean_frac_clusters=mean(frac_clusters),
         #                  se_frac_clusters=sd(frac_clusters)/sqrt(n())),
  ...
  # geom_errorbar(aes(ymin=100*(mean_frac_clusters-se_frac_clusters),
  #                   ymax=100*(mean_frac_clusters+se_frac_clusters)), width=0,
  #               position=position_dodge(width=.8)) +
  my_theme() +
  scale_fill_manual(values=c('gray','lightcyan')) +
  #scale_color_manual(values=c('gray','coral')) +
  #scale_color_brewer(palette='Set1') +
  scale_color_manual(values=setNames(brewer.pal(3,'Set1'),c('A','B','C')),na.value='gray20') +
  #coord_cartesian(ylim=c(0,100)) +
  labs(x='# of cells',y='% shared clusters in pair', fill='pair', color='donor')
```
...
```{r FigS4X2, fig.width=2 * fig_scale, fig.height=2.5 * fig_scale}
...
col.order <- gsub('_downsampled|_switch','',tmp$col[order(tmp$coef)])
...
tmp %>%
  ...
  ggplot(aes(x=1, y=col, fill=coef, label=round(coef,2))) + 
  geom_tile() +
  scale_fill_gradient2(low='pink2',high='steelblue3', mid='white', breaks=c(-.05, 0, .05)) +
  my_theme() +
  coord_cartesian(clip='off') +
  ...
  annotation_custom(grob=anno_grob_y, ymin = 0, ymax = 1, xmin = -3.5, xmax = 4) +
  labs(x='',y='') 
```
