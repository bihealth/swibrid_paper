...
wb <- createWorkbook()
addWorksheet(wb, sheetName='Fig1D')
addWorksheet(wb, sheetName='Fig1E')
addWorksheet(wb, sheetName='Fig1F')
addWorksheet(wb, sheetName='Fig1G')
addWorksheet(wb, sheetName='Fig2B')
addWorksheet(wb, sheetName='Fig2C')
addWorksheet(wb, sheetName='Fig2D')
addWorksheet(wb, sheetName='Fig2E')
addWorksheet(wb, sheetName='Fig2F')
addWorksheet(wb, sheetName='Fig2G')
addWorksheet(wb, sheetName='Fig2H')
addWorksheet(wb, sheetName='Fig3B')
addWorksheet(wb, sheetName='Fig3C')
addWorksheet(wb, sheetName='Fig3D')
addWorksheet(wb, sheetName='Fig3E')
addWorksheet(wb, sheetName='Fig3F')
addWorksheet(wb, sheetName='Fig3G')
addWorksheet(wb, sheetName='Fig4B')
addWorksheet(wb, sheetName='Fig4C')
addWorksheet(wb, sheetName='Fig4D')
addWorksheet(wb, sheetName='Fig4E')
addWorksheet(wb, sheetName='Fig4F')
addWorksheet(wb, sheetName='Fig4G')
addWorksheet(wb, sheetName='Fig4H')
addWorksheet(wb, sheetName='Fig4J')
addWorksheet(wb, sheetName='FigS2F')
addWorksheet(wb, sheetName='FigS2G')
addWorksheet(wb, sheetName='FigS2H')
addWorksheet(wb, sheetName='FigS2I')
addWorksheet(wb, sheetName='FigS2J')
addWorksheet(wb, sheetName='FigS3A')
addWorksheet(wb, sheetName='FigS3B')
addWorksheet(wb, sheetName='FigS3C')
addWorksheet(wb, sheetName='FigS3D')
addWorksheet(wb, sheetName='FigS4A')
addWorksheet(wb, sheetName='FigS4B')
addWorksheet(wb, sheetName='FigS4C')
addWorksheet(wb, sheetName='FigS4D')
addWorksheet(wb, sheetName='FigS4E')
addWorksheet(wb, sheetName='FigS4F')
addWorksheet(wb, sheetName='FigS4G')
addWorksheet(wb, sheetName='FigS4H')
```
...
synthetic.data <- read.xlsx('data/input_data.xlsx',sheet='synthetic data')
dat <- synthetic.data %>% 
         dplyr::mutate(nreads=factor(nreads, levels=c('10k','20k','50k','100k'))) %>%
  dplyr::select(nclones, clusters_raw, nreads)
ggplot(dat, aes(x=nclones, y=clusters_raw, color=nreads)) + 
...
writeData(wb, 'Fig1D', as.data.frame(dat))
```
...
cellswitch.data <- read.xlsx('data/input_data.xlsx',sheet='cell numbers')
dat <- cellswitch.data %>%
         dplyr::mutate(Donor=plyr::revalue(Donor,
                                           c('21084'='A',
                                             '21085'='B',
                                             '21086'='C'))) %>%
  dplyr::select(ncells, clusters_raw, Donor)
ggplot(dat, aes(x=ncells, y=clusters_raw)) + 
  geom_abline(slope=1,intercept=0, size=.5, color='gray80') +
...
dat <- meta_clustering %>% 
  ...
  dplyr::select(ncells, frac_clusters, mixing, Donor)
ggplot(dat, aes(x=ncells,y=100*frac_clusters, fill=mixing)) + 
...
writeData(wb, 'Fig1F', as.data.frame(dat))
```
...
VDJ_results <- read.xlsx('data/input_data.xlsx',sheet='VDJ data')
...
dat <- vdj_df %>% 
  ...
  dplyr::select(mean_alpha_ratio_VDJ,
                se_alpha_ratio_VDJ,
                mean_alpha_ratio_SWIBRID,
                se_alpha_ratio_SWIBRID,
                Donor, ncells)

ggplot(dat, aes(x=mean_alpha_ratio_VDJ, y=mean_alpha_ratio_SWIBRID,
...
writeData(wb, 'Fig1G', as.data.frame(dat))
```
...
# HD data for Fig. 2
HD.data <- read.xlsx('data/input_data.xlsx', sheet='HD data')
...
dat <- HD.var %>%  
  ...
  dplyr::select(col, variance, variable, category)

ggplot(dat, aes(x=col, y=100*variance, fill=variable)) + 
  geom_col(width=.8) +
  my_theme() +
...
writeData(wb, 'Fig2B', as.data.frame(dat))
```
...
ggplot(dat, aes(x=variable, y=100*variance)) + 
...
writeData(wb, 'Fig2C', as.data.frame(dat))
```
...
dat <- HD.var %>%  
  ...
  dplyr::select(category, variance, variable, col)
ggplot(dat, aes(x=category, y=100*variance, fill=variable)) + 
...
writeData(wb, 'FigS2F', as.data.frame(dat))
```
...
ggplot(dat1,aes(x=variable, y=100*variance, fill=category)) + 
  geom_boxplot(position=position_dodge(width=.8), outlier.shape=NA, lwd=.5) + 
...
ggplot(dat2, aes(x=variable, y=100*variance, fill=category)) + 
  geom_boxplot(position=position_dodge(width=.8), outlier.shape=NA, lwd=.5) + 
  geom_jitter(position=position_jitterdodge(jitter.width=.3,dodge.width=.8), size=.5) +
  geom_text(data=pvals, aes(x=variable, label=stars.pval(p)), y=Inf, inherit.aes=FALSE, size=5) +
  my_theme() +
...
HD.data.br <- read.xlsx('data/input_data.xlsx', sheet='HD data by reads')
...
dat3 <- rbind(HD.var %>% 
  ...
  dplyr::mutate(category=averaging) %>%
  dplyr::select(col, variable, variance, category) %>%
  dplyr::arrange(col,category,variable) 

pvals <- dat3 %>%
  dplyr::group_by(variable) %>%
  dplyr::do(wc=wilcox.test(variance ~ category, paired=TRUE, data=., exact=FALSE)) %>%
  dplyr::summarise(variable, p=wc$p.value)

ggplot(dat3, aes(x=variable, y=100*variance, fill=category)) + 
  geom_boxplot(position=position_dodge(width=.8), outlier.shape=NA, lwd=.5) + 
...
writeData(wb, 'FigS2I', rbind(dat1 %>% dplyr::mutate(panel='FigS2I_1'), 
                              dat2 %>% dplyr::mutate(panel='FigS2I_2'), 
                              dat3 %>% dplyr::mutate(panel='FigS2I_3')) %>% as.data.frame())
```
...
dat <- HD.data %>%
  dplyr::filter(feature %in% c('break_dispersion_SG1','occupancy_top_cluster'), Cohort=='C1') %>%
  dplyr::select(Age, value, feature, Sex)

dat %>% dplyr::filter(feature == 'break_dispersion_SG1') %>%
  ...
  labs(x='',y='break_dispersion_SG1')

dat %>% dplyr::filter(feature == 'occupancy_top_cluster') %>%
  ...
  theme(legend.position='none',
        axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(x='',y='occupancy_top_cluster')

writeData(wb, 'Fig2E', as.data.frame(dat))
```
...
ggplot(dat, aes(x=var1,y=var2,fill=R)) + 
  geom_tile() +
...
writeData(wb, 'FigS2H', as.data.frame(dat)) 
```
...
dat <- cohort.data %>%
  dplyr::select(Cohort, value, Donor, feature, category)
ggplot(dat, aes(x=Cohort, y=value)) + 
...
writeData(wb, 'Fig2D', as.data.frame(dat))
```

same thing for mouse tissue and Vincendeau

```{r Fig2F, fig.width=4. * fig_scale, fig.height=1.75 * fig_scale}
mouse.data <- read.xlsx('data/input_data.xlsx', sheet='mouse tissues') %>%
  tibble::column_to_rownames('Sample')
vincendeau.data <- read.xlsx('data/input_data.xlsx', sheet='Vincendeau data') %>%
  tibble::column_to_rownames('Sample')
...
dat <- cohort.data %>%
  dplyr::select(Cohort, value, species, variable)

ggplot(dat, aes(x=Cohort, y=value, fill=species)) + 
...
writeData(wb, 'Fig2F', as.data.frame(dat))
```
...
writeData(wb, 'FigS2J', as.data.frame(dat))
```
...
dat <- HD.scaled %>%
  ...
  dplyr::mutate(feature=factor(feature, levels=columns %>% 
                                 ...
                                 dplyr::pull(col))) %>%
  dplyr::select(minION, pacBio, feature, category) %>%
...
cols_shapes <- dat %>% 
  ...
  dplyr::mutate(feature=factor(feature, levels=levels(dat$feature))) %>% 
  dplyr::arrange(feature)

ggplot(dat,aes(x=minION,y=pacBio)) +
...
writeData(wb, 'FigS2G', as.data.frame(dat))
```
...
dat <- pcadat %>%
  dplyr::select(PC1, PC2, Genotype)

ggplot(dat,aes(x=PC1,y=PC2, color=Genotype)) +
  geom_point(size=1) +
...
writeData(wb, 'Fig2G', as.data.frame(dat), rowNames=TRUE)
```
...
dat <- tmp %>%
  dplyr::select(-c(var_col,category))
...
writeData(wb, 'Fig2H', as.data.frame(dat))
```
# Figure 3 + supplement (CH12)

```{r FigS3A, fig.width=5.5 * fig_scale, fig.height=4.5 * fig_scale}
HTGTS.data <- read.xlsx("data/input_data.xlsx", sheet='HTGTS data') %>%
  tibble::column_to_rownames('Sample')
...
cols_shapes <- dat %>% 
  ...
  dplyr::group_by(category) %>%
  dplyr::mutate(var_ind=1:n()) %>%
  dplyr::mutate(variable=factor(variable, levels=levels(dat$variable))) %>% 
  dplyr::arrange(variable)
...
writeData(wb, 'FigS3A', dat %>% dplyr::select(-c(category, var_col)) %>% as.data.frame())
```
...
dat <- HTGTS.data %>%
  ...
  dplyr::filter(category %in% c('Diversity','Isotypes','Breaks','Context','Structural')) %>%
  dplyr::select(Genotype, value, celltype, variable)
...
dat <- read.xlsx('data/input_data.xlsx', sheet='HTGTS homology') %>%
  ...
  dplyr::select(n_homology, mean_frac, se_frac, Genotype, celltype)

ggplot(dat, aes(x=n_homology, y=mean_frac, color=Genotype)) +
...
writeData(wb, 'FigS3D', as.data.frame(dat))
```
...
dat <- rbind(pcadat.CH12 %>% dplyr::mutate(material='CH12') %>%
               dplyr::select(PC1, PC2, Genotype, material),
             pcadat.spleen %>% dplyr::mutate(material='spleen') %>%
               dplyr::select(PC1, PC2, Genotype, material))

p1 <- ggplot(dat %>% dplyr::filter(material=='CH12'),aes(x=PC1,y=PC2, color=Genotype)) +
  geom_point(size=1) +
  geom_polygon(data = dat %>% dplyr::filter(material=='CH12') %>%
                 dplyr::group_by(Genotype) %>%
                 dplyr::slice(chull(PC1, PC2)), alpha = 0.25, aes(fill=Genotype)) +
  my_theme() +
...
p2 <- ggplot(dat %>% dplyr::filter(material=='spleen'),aes(x=PC1,y=PC2, color=Genotype)) +
  geom_point(size=1) +
  geom_polygon(data = dat %>% dplyr::filter(material=='spleen') %>%
                 dplyr::group_by(Genotype) %>%
                 dplyr::slice(chull(PC1, PC2)), alpha = 0.25, aes(fill=Genotype)) +
  my_theme() +
...
writeData(wb, 'FigS3B', as.data.frame(dat), rowNames=TRUE)
```
...
CH12.data <- read.xlsx('data/input_data.xlsx',sheet='CH12 data') %>%
  tibble::column_to_rownames('Sample')
CH12.data.noSg <- read.xlsx('data/input_data.xlsx',sheet='CH12 data (no Sg)') %>%
  tibble::column_to_rownames('Sample')
mouse.data.noSg <- read.xlsx('data/input_data.xlsx',sheet='mouse tissues (no Sg)')
...
dat <- rbind(pcadat %>%
               dplyr::mutate(set='all regions')  %>%
               dplyr::select(PC1, PC2, material, Genotype, set),
             pcadat.noSg %>%
               dplyr::mutate(set='no Sg') %>%
               dplyr::select(PC1, PC2, material, Genotype, set))

p1 <- ggplot(dat %>% dplyr::filter(set=='all regions'),aes(x=PC1,y=PC2, color=material)) +
  geom_point(aes(shape=Genotype),size=1) +
  geom_polygon(data = dat %>%
                 dplyr::filter(set=='all regions') %>%
...
p2 <- ggplot(dat %>% dplyr::filter(set=='no Sg'),aes(x=PC1,y=PC2, color=material)) +
  geom_point(aes(shape=Genotype), size=1) +
  geom_polygon(data = dat %>%
                 dplyr::filter(set=='no Sg') %>%
                 dplyr::group_by(material) %>%
                 dplyr::slice(chull(PC1, PC2)), alpha = 0.25, aes(fill=material)) +
  my_theme() +
  scale_fill_manual(values=tissue_colors) +
...
writeData(wb, 'Fig3B', as.data.frame(dat), rowNames=TRUE)
```
...
dat <- CH12.data %>% 
  ...
  dplyr::select(Genotype, value, col)
...
writeData(wb, 'Fig3D', as.data.frame(dat))
```
...
writeData(wb, 'Fig3E', as.data.frame(dat), rowNames=TRUE)
```
...
dat <- rbind(t(CH12.meta[samples, c('Genotype','Batch')]),
             t(CH12.X[samples,CH12.cols]))

writeData(wb, 'Fig3C', as.data.frame(dat), rowNames=TRUE)
```
...
dat <- as.data.frame(predict(mod, type = "response", 
  ...
  dplyr::select(prediction, sample, p, c)

ggplot(dat, aes(x=prediction,y=sample,
                label=round(p,2),fill=p,color=c)) + 
  geom_tile() + 
  geom_text(data=dat %>% dplyr::filter(p > .25),
            size=2) +
...
writeData(wb, 'Fig3G', as.data.frame(dat))
```
...
dat <- tmp %>%
  ...
  dplyr::select(Genotype, col, coef)

ggplot(dat, aes(x=Genotype, y=col, fill=coef, label=round(coef,2))) + 
  geom_tile() +
...
writeData(wb, 'Fig3F', as.data.frame(dat))
```
...
FB.data <- read.xlsx('data/input_data.xlsx', sheet='CVID data') %>%
  dplyr::mutate(Batch=as.character(Batch)) %>%
  tibble::column_to_rownames('Sample')
...
writeData(wb, 'Fig4B', as.data.frame(dat)) 
```
...
FB.data.pooled <- read.xlsx('data/input_data.xlsx',sheet='CVID data pooled') %>%
  ...
  tibble::column_to_rownames('Sample')
QP.data <- read.xlsx('data/input_data.xlsx', sheet='DNA repair data') %>%
...
dat <- pcadat %>% 
  dplyr::select(PC1, PC2, Diagnosis, Donor, status)
...
writeData(wb, 'Fig4D', as.data.frame(dat), rowNames=TRUE)
```
...
dat <- FB.data.pooled %>%
  ...
  dplyr::select(feature, Diagnosis, frac_clusters, Donor)
...
ggplot(dat, aes(x=feature, y=frac_clusters)) +
  ...
  geom_line(data=dat %>% dplyr::filter(Diagnosis=='Control'),
            aes(group=Donor), position=position_nudge(x=-.2), alpha=.25, size=.25) +
  geom_line(data=dat %>% dplyr::filter(Diagnosis=='Ssubclass'),
            aes(group=Donor), position=position_nudge(x=+.2), alpha=.25, size=.25) +
...
writeData(wb, 'FigS4D', as.data.frame(dat))
```
...
dat <- tmp %>%
  dplyr::select(alpha_ratio_FACS, alpha_ratio_clusters, status, Donor)

ggplot(dat, aes(x=alpha_ratio_FACS, y=alpha_ratio_clusters, color=status)) + 
...
writeData(wb, 'FigS4E', as.data.frame(dat))
```
...
dat <- FB.data.pooled[FB.data.pooled$Sample_donor %in% FB_train_donors,] %>% 
  ...
  dplyr::select(Diagnosis_simple, value, col,Donor)

ggplot(dat, aes(x=Diagnosis_simple,y=value,fill=Diagnosis_simple)) + 
  geom_boxplot(outlier.shape=NA, lwd=.5) + 
  geom_jitter(width=.1,height=0,size=.5) + 
...
writeData(wb, 'Fig4C', as.data.frame(dat))
```
...
dat <- pcadat.combined %>%
  ...
  dplyr::select(PC1, PC2, dataset, status, Diagnosis, Donor)

ggplot(dat, aes(x=PC1,y=PC2)) +
  geom_polygon(data = dat %>% dplyr::filter(dataset=='training') %>%
                 dplyr::group_by(status) %>%
...
writeData(wb, 'Fig4E', as.data.frame(dat), rowNames=TRUE)
```
...
writeData(wb, 'Fig4F', as.data.frame(dat), rowNames=TRUE)
```
...
dat <- data.frame(specificity=c(ROC_1$specificities,ROC_2$specificities),
  ...
  dplyr::select(specificity, sensitivity, model)

ggplot(dat, aes(x=specificity,y=sensitivity,color=model)) + 
  geom_step() +
  scale_x_reverse() +  
...
writeData(wb, 'Fig4G', as.data.frame(dat))
```
...
dat <- tmp %>%
  ...
  dplyr::select(col, coef)

ggplot(dat, aes(x=col, y=1, fill=coef, label=round(coef,2))) + 
  geom_tile() +
...
writeData(wb, 'FigS4G', as.data.frame(dat))
```
...
hm_meta <- FB.meta[,c('Diagnosis','Sex','Age','Donor','Timepoint'),drop=FALSE] %>% 
...
writeData(wb, 'Fig4H', as.data.frame(dat), rowNames=TRUE)
```
...
dat <- tmp %>%
  dplyr::select(PC1, PC2, col)

ggplot(dat,aes(x=abs(PC1),y=abs(PC2), color=col, shape=col)) +
  geom_point(size=1) +
  my_theme() +
  scale_color_manual(values=cols_shapes %>%
...
writeData(wb, 'FigS4C', as.data.frame(dat))
```

categorize the samples

```{r FigS4H, fig.width=2.5 * fig_scale, fig.height=2 * fig_scale}
dat <- pca_hm %>% tibble::rownames_to_column('name') %>%
  dplyr::filter(dataset!='validation') %>%
  dplyr::left_join(class_df, by='name') %>%
  dplyr::select(PC1, PC2, Class, dataset, Donor)

ggplot(dat, aes(x=PC1,y=PC2, color=Class, shape=dataset)) +
  geom_point(size=1) +
...
writeData(wb, 'FigS4H', as.data.frame(dat))
```
...
dat <- cbind(rbind(FB.data.pooled[,features.here],
  ...
  dplyr::select(group, value, col, Donor)


ggplot(dat, aes(x=ifelse(grepl('Cluster',group),as.numeric(group),1+as.numeric(group)),
                y=value,fill=group)) +
  geom_boxplot(outlier.shape=NA, lwd=.5) +
  geom_jitter(width=.1,height=0,size=.5) +
...
writeData(wb, 'Fig4J', as.data.frame(dat))
```
...
dat <- FB.data.pooled[FB.samples, ] %>%
  ...
  dplyr::select(vacc_status, value, feature, vacc, status)

ggplot(dat, aes(x=vacc_status, y=value, fill=vacc_status)) +
...
writeData(wb, 'FigS4B', as.data.frame(dat))
```
...
dat <- rbind(t(FB.meta.aggregated[samples,]),
             t(FB.X.aggregated[samples,]))

writeData(wb, 'FigS4F', as.data.frame(dat), rowNames=TRUE)
```

```{r save}
saveWorkbook(wb, file = "data/source_data_file.xlsx", overwrite = TRUE)
```
