...
<!-- ```{r get_data} -->
<!-- # data from Vincendeau for Fig. 2 -->
<!-- # mouse tissue data for Figs. 2 + 3  -->
<!-- # CH12 data for Fig. 3 -->
<!-- # patient data for Fig. 4 -->
...
```{r FigS3A,fig.width=3.5 * fig_scale,fig.height=1.7 * fig_scale}
HD.var %>%  
  dplyr::left_join(columns, by='col') %>%
...
```{r FigS3D_1, fig.width=1.4 * fig_scale, fig.height=1.7 * fig_scale}
tmp <- HD.var %>%  
  dplyr::left_join(columns, by='col') %>%
  dplyr::mutate(variable=factor(gsub('log10_nreads','# reads',variable),
                                levels=rev(c('donor','cohort','batch','sequencing','# reads','Residuals')))) %>%
  dplyr::filter(variable!='Residuals', category %in% c('diversity','diversity_raw'), variable!='cohort') 

pvals <- tmp %>%
  dplyr::group_by(variable) %>%
  do(wc=wilcox.test(variance ~ category, data=.)) %>%
  summarise(variable, p=wc$p.value)
...
```{r FigS3D_2, fig.width=1.4 * fig_scale, fig.height=1.7 * fig_scale}
tmp <- HD.var %>%  
  dplyr::left_join(columns, by='col') %>%
  dplyr::mutate(variable=factor(gsub('log10_nreads','# reads',variable),
                                levels=rev(c('donor','cohort','batch','sequencing','# reads','Residuals')))) %>%
...
pvals <- tmp %>% 
  dplyr::group_by(variable) %>%
  do(wc=wilcox.test(variance ~ category, data=., exact=FALSE)) %>%
  summarise(variable, p=wc$p.value)
...
```{r FigS3D_3, fig.width=1.4 * fig_scale,fig.height=1.7 * fig_scale}
...
pvals <- tmp %>%
  dplyr::group_by(variable) %>%
  do(wc=wilcox.test(variance ~ averaging, paired=TRUE, data=., exact=FALSE)) %>%
  summarise(variable, p=wc$p.value)
...
```{r FigS3B, fig.width=4.25 * fig_scale, fig.height=4.25 * fig_scale}
library(dendextend)
FB.X <- HD.data %>%
...
```{r FigS3E, fig.width=1.5 * fig_scale, fig.height=1.75 * fig_scale}
...
```{r FigS3C, fig.width=2.75 * fig_scale, fig.height=5 * fig_scale}
tmp <- HD.scaled %>%
...
# ggplot(pcadat,aes(x=PC3,y=PC4, color=genotype)) +
#   geom_point(size=2) +
#   geom_polygon(data = pcadat %>%
#                  dplyr::group_by(genotype) %>%
#                  dplyr::slice(chull(PC3, PC4)), alpha = 0.25, aes(fill=genotype)) +
#   my_theme() +
#   theme(aspect.ratio=pca$sdev[4]**2/pca$sdev[3]**2) +
#   labs(x=paste0('PC 3 (',100*signif(pca$sdev[3]**2/sum(pca$sdev**2),3),'%)'),
#        y=paste0('PC 4 (',100*signif(pca$sdev[4]**2/sum(pca$sdev**2),3),'%)'),
#        color='',shape='',fill='') +
#   scale_fill_manual(values=genotype_colors) +
#   scale_color_manual(values=genotype_colors)
# 
# ggplot(pcadat,aes(x=PC1,y=PC2, color=batch)) +
#   geom_point(size=2) +
#   my_theme() +
#   theme(aspect.ratio=pca$sdev[2]**2/pca$sdev[1]**2) +
#   labs(x=paste0('PC 1 (',100*signif(pca$sdev[1]**2/sum(pca$sdev**2),3),'%)'),
...
```{r FigS3G, fig.width=1. * fig_scale, fig.height=1.75 * fig_scale}
batch.CV <- c()
...
```{r FigS4X1, fig.width=2.25 * fig_scale, fig.height=2 * fig_scale}
...
```{r FigS4G, fig.width=2 * fig_scale, fig.height=1.7 * fig_scale}
...
ggplot(tmp, aes(x=feature, y=frac_clusters, color=diagnosis)) +
  geom_boxplot(position=position_dodge(width=.8),width=.7,outlier.shape=NA) +
  geom_point(position=position_dodge(width=.8), size=.5) +
...
```{r FigS4X2, fig.width=2.5 * fig_scale, fig.height=2 * fig_scale}
FB.data.pooled %>%
  dplyr::mutate(alpha_ratio_FACS=pct_IgA/(pct_IgA + pct_IgG)) %>%
  ggplot(aes(x=alpha_ratio_FACS, y=alpha_ratio_clusters, color=status)) + 
...
project new data into the old PCA
...
```{r Fig4G, fig.width=2.25 * fig_scale, fig.height=2 * fig_scale}
...
```{r Fig4H, fig.width=2.5 * fig_scale, fig.height=1.5 * fig_scale}
set.seed(1)
...
FB_res <- data.frame(specificity=c(ROC_1$specificities,ROC_2$specificities),
                     ...
                     model=c(rep(paste0('training -> testing; AUC=',signif(auc(ROC_1),3)),length(ROC_1$specificities)),
                             rep(paste0('training -> validation; AUC=',signif(auc(ROC_2),3)),length(ROC_2$specificities)))) %>%
...
```{r FigS4X3, fig.width=.8 * fig_scale, fig.height=1 * fig_scale}
set.seed(4)
sample.CV <- c()
...
```{r Fig4F, fig.width=5.5 * fig_scale, fig.height=5.5 * fig_scale}
...
```{r FigS4C, fig.width=3 * fig_scale, fig.height=3 * fig_scale}
tmp <- data.frame(abs(pca$rotation)) %>%
  tibble::rownames_to_column('col') %>%
  dplyr::left_join(columns, by='col') %>%
...
```{r FigS4D, fig.width=3 * fig_scale, fig.height=1.5 * fig_scale}
ggplot(pca_hm %>% tibble::rownames_to_column('name') %>%
  ...
  my_theme() +
  scale_shape_manual(values=c('testing'=15,'training'=16,'validation'=18)) +
...
```{r Fig4J, fig.width=4.5 * fig_scale, fig.height=1.7 * fig_scale}
features.here <- c('frac_clusters_SG2',
                   'mean_cluster_size_downsampled',
                   'alpha_ratio_clusters',
                   'n_homology_switch',
                   'frac_breaks_duplications')
...
```{r clinical_features, fig.width=3 * fig_scale, fig.height=2.5 * fig_scale}
pvals <- list()
for (x in c("Tetanus",
            "Diphteria",
            "PnPS")) {
...
```{r aggregated_FB_heatmap, fig.width=5.5 * fig_scale, fig.height=3.5 * fig_scale}
FB.X.aggregated <-  FB.data.pooled[FB.samples, selected.cols] %>%
  ...
  #dplyr::filter(diagnosis!='control') %>%
  dplyr::select(sample,category,value) %>%
  spread(category,value) %>%
  tibble::column_to_rownames('sample')

hm2 <- Heatmap(t(FB.X.aggregated),
               #cluster_columns=dend,
               col=colorRamp2(c(0, 1.5), c("lightyellow2","steelblue3")),
               cluster_rows=TRUE,
               show_row_names=TRUE,
               show_column_names=TRUE,
               show_row_dend=TRUE,
               top_annotation=HeatmapAnnotation(df=FB.meta[row.names(FB.X.aggregated),c('diagnosis','sex','age',
                                                                                        'donor','timepoint'),drop=FALSE] %>% 
                                                  tibble::remove_rownames() %>%
                                                  dplyr::mutate(name=gsub('t','',gsub('FB_','',paste0(donor,'_',timepoint)))) %>%
                                                  dplyr::left_join(class_df,by='name') %>%
...
what drives clustering within clusters 1 + 2?

```{r cluster_drivers, fig.width=3 * fig_scale, fig.height=3 * fig_scale}
use <- class_df$class!='cluster3'
cluster.var <- fitExtractVarPartModel(t(FB.X[use,]), ~ (1 | class), data=class_df[use,'class',drop=FALSE]) %>%
  as.data.frame() %>%
  tibble::rownames_to_column('col') %>%
  gather(variable,variance,-col)
...
cluster.var %>%
  ...
  dplyr::left_join(columns %>% dplyr::mutate(col=gsub('_downsampled|_switch','',col)) %>% 
                     dplyr::filter(category %in% c('context','breaks','isotypes','diversity','structural')), 
                   by='col') %>%
  dplyr::arrange(variance) %>%
  dplyr::mutate(col=factor(col, levels=col)) %>%
  ggplot(aes(y=variance, x=col, fill=category)) + 
  geom_col() +
  my_theme() +
  coord_flip() +
  scale_fill_manual(values=cat_cols) +
  labs(x='',y='explained variance between cluster1+2 by class') +
  theme(legend.position=c(.9,.1))
```

box plots for selected features

```{r Fig4K, fig.width=4. * fig_scale, fig.height=1.7 * fig_scale}
