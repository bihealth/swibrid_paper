...
<!-- human.X.subclass <- cbind(rbind(FB.data.pooled[,FB.cols.subclass], -->
<!--                                 QP.data[,FB.cols.subclass]), -->
<!--                           human.meta[,'dataset',drop=FALSE]) %>% -->
<!--   tibble::rownames_to_column('sample') %>% -->
<!--   dplyr::mutate(sample=gsub('[0-9]*_|pooled_','',sample)) %>% -->
<!--   dplyr::group_by(dataset) %>% -->
<!--   dplyr::mutate_if(is.numeric,scale) %>% -->
<!--   dplyr::ungroup() %>% -->
<!--   dplyr::select(c('sample',FB.cols.subclass)) %>% -->
<!--   tibble::column_to_rownames('sample') -->

<!-- pca.subclass <- prcomp(human.X.subclass[train,]) -->
...
<!--                  #dplyr::mutate(status=ifelse(Diagnosis %in% c('Subclass','Atypical'), as.character(Diagnosis), as.character(status))) %>% -->
<!--                  dplyr::group_by(status) %>% -->
...
<!-- (this PCA uses ` dim(human.X.subclass)[2]` features) -->

show subclass features for HD and subclass-deficient people

```{r FigS4D, fig.width=2 * fig_scale, fig.height=1.7 * fig_scale}
...
```{r FigS4E, fig.width=2.25 * fig_scale, fig.height=2 * fig_scale}
tmp <- FB.data.pooled %>%
...
<!-- project into PCA using only subclass features -->

<!-- ```{r FigS4X5, fig.width=2.25 * fig_scale, fig.height=2 * fig_scale} -->
<!-- pcadat.combined.subclass <- rbind(cbind(as.matrix(human.X.subclass[train,]) %*% pca.subclass$rotation, human.meta[train,]), -->
<!--                                   cbind(as.matrix(human.X.subclass[test,]) %*% pca.subclass$rotation, human.meta[test,]), -->
<!--                                   cbind(as.matrix(human.X.subclass[validation,]) %*% pca.subclass$rotation, human.meta[validation,])) %>% -->
<!--   dplyr::mutate(dataset=factor(dataset, levels=c('training','testing','validation'))) -->

<!-- ggplot(pcadat.combined.subclass, -->
<!--        aes(x=PC1,y=PC2)) + -->
<!--   geom_polygon(data = pcadat.combined.subclass %>% dplyr::filter(dataset=='training') %>% -->
<!--                  dplyr::group_by(status) %>% -->
<!--                  dplyr::slice(chull(PC1, PC2)), alpha = 0.1, color=NA, aes(fill=status), show.legend=FALSE) + -->
<!--   geom_point(aes(color=Diagnosis, alpha=dataset, shape=dataset), size=1) + -->
<!--   geom_line(aes(group=Donor, color=Diagnosis,alpha=dataset, shape=dataset)) + -->
...
<!--   scale_fill_manual(values=diagnosis_colors, guide='none') + -->
<!--   scale_color_manual(values=diagnosis_colors) + -->
<!--   scale_alpha_manual(values=c('testing'=1,'training'=.25,'validation'=1)) + -->
<!--   scale_shape_manual(values=c('testing'=15,'training'=16,'validation'=4)) + -->
<!--   guides(color=guide_legend(override.aes = list(shape=15,size=2), ncol=3), -->
<!--          shape=guide_legend(ncol=1), -->
<!--          alpha=guide_legend(ncol=1)) + -->
<!--   theme(legend.key.height=unit(1.5,'mm'), -->
...
<!-- regression using only subclass features -->

<!-- ```{r FigS4X6, fig.width=2. * fig_scale, fig.height=1.5 * fig_scale, include=FALSE} -->
<!-- set.seed(1) -->
<!-- train.here <- train & !(human.meta[,'Diagnosis'] %in% c('Subclass','Atypical'))  -->
<!-- test.here <- test & !(human.meta[, 'Donor'] %in% FB_train_donors) -->
<!-- cv_model <- cv.glmnet(as.matrix(human.X.subclass[train.here,]), human.meta[train.here,'status']!='Control', alpha = 0, family='binomial') -->
<!-- mod <- glmnet(as.matrix(human.X.subclass[train.here,]), human.meta[train.here,'status']!='Control', alpha = 0, family = "binomial", -->
<!--                 lambda = cv_model$lambda.min) -->
<!-- pred_1 <- predict(mod, type = "response", newx=as.matrix(human.X.subclass[test.here,])) -->
<!-- ROC_1 <- roc(human.meta[test.here,'status']!='Control', pred_1) -->

<!-- pred_2 <- predict(mod, type = "response", newx=as.matrix(human.X.subclass[validation,])) -->
<!-- ROC_2 <- roc(human.meta[validation,'status']!='Control', pred_2) -->
...
ggplot(tmp, aes(x=ifelse(dataset=='CH12',as.numeric(group),as.numeric(group) + 1),y=value,fill=group)) +
  ...
  stat_compare_means(comparisons=list(c(7,8),
                                      ...
                                      c(7,11)),
                     method='wilcox.test',label = "p.signif", hide.ns=TRUE, size=2, vjust=.5) +
  facet_wrap(.~col,ncol=6,scales='free_y') +
  my_theme() +
  coord_cartesian(clip='off') +
  scale_fill_manual(values=c(diagnosis_colors,genotype_colors)) +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=.5),
...
```{r inhibitors, fig.width=6.75 * fig_scale, fig.height=6.5 * fig_scale }
inhibitors_sample_sheet <- read.csv('../sodar/2019_Inhibitors/a_2019_Inhibitors.txt',
                                    sep='\t',header=1) %>%
  dplyr::left_join(read.csv('../sodar/2019_Inhibitors/s_2019_Inhibitors.txt',sep='\t',header=1),
                   by='Sample.Name') %>%
  dplyr::mutate(Donor=Source.Name,
                Treatment=Parameter.Value.Treatment.,
                Dose=Parameter.Value.Dose.,
                Batch=Parameter.Value.Batch.,
                Sample=Comment.Extract.Name.) %>%
  dplyr::distinct(Donor,Treatment,Dose,Sample,Batch,.keep_all = TRUE)

inhibitors.data <- read.csv('../human_samples/20240307_hg38_results/output/summary/20240307_human_samples_stats.csv',
                            header=1,row.names=1) %>%
  tibble::rownames_to_column('Sample') %>%
  dplyr::inner_join(inhibitors_sample_sheet %>% dplyr::distinct(Donor, Treatment, Sample, Batch), by='Sample') %>%
  dplyr::filter(nreads_final >= 500, frac_clusters_SA1+frac_clusters_SA2 > .05,
                frac_clusters_SG1 + frac_clusters_SG2 + frac_clusters_SG3 + frac_clusters_SG4 > .05) %>%
  dplyr::mutate_all(~replace_na(.,0)) %>%
  dplyr::mutate(Diagnosis=Treatment) %>%
  dplyr::select(tidyselect::any_of(c('Donor','Diagnosis','Sample','Batch',columns$col))) %>%
  dplyr::mutate(num_variants_somatic=num_variants * (1 - frac_variants_germline))

# inhibitors.X <- inhibitors.data[,selected.cols] %>% dplyr::mutate_if(is.numeric, scale)
# inhibitors.meta <- inhibitors.data[,c('Donor','Diagnosis')] %>%
#   dplyr::mutate(Diagnosis=ifelse(grepl('BIBR',Diagnosis),
#                                  'BIBR',
#                                  ifelse(grepl('ATM',Diagnosis),'ATMi',
#                                         ifelse(grepl('DMSO',Diagnosis),'DMSO',Diagnosis))))
# 
# pcadat <- cbind(prcomp(inhibitors.X)$x, inhibitors.meta)
# 
# ggplot(pcadat, aes(x=PC1,y=PC2)) +
#   geom_point(aes(color=Diagnosis), size=2) +
#   my_theme() +
#   scale_color_manual(values=c(diagnosis_colors,c('DMSO'='gray40','HU'='orange1','Olaparib'='blue2','SRC7'='orchid1',
#                                                  'DSMO'='gray40','SCR7'='orchid1','BIBR'='green3','ATMi'='brown4'))) +
#   guides(color=guide_legend(override.aes = list(shape=15,size=2), ncol=3)) +
#   theme(legend.key.height=unit(1.5,'mm'),
#         legend.key.spacing=unit(0,'mm'),
#         legend.position='bottom',
...
#        color='',shape='',fill='',alpha='')
# 
# pcadat.test <- rbind(cbind(as.matrix(human.X[train,]) %*% pca$rotation, human.meta[train,c('Donor','Diagnosis')]),
#                      cbind(as.matrix(human.X[test,]) %*% pca$rotation, human.meta[test,c('Donor','Diagnosis')]),
#                      cbind(as.matrix(inhibitors.X) %*% pca$rotation,inhibitors.meta)) %>%
#   dplyr::mutate(dataset=ifelse(grepl('FB',Donor),'FB','inhibitors'))
# 
# ggplot(pcadat.test, aes(x=PC1,y=PC2)) +
#   geom_polygon(data = pcadat.test %>% dplyr::filter(dataset=='FB', Diagnosis %in% c('CVID','Control')) %>%
#                  dplyr::group_by(Diagnosis) %>%
#                  dplyr::slice(chull(PC1, PC2)), alpha = 0.1, color=NA, aes(fill=Diagnosis), show.legend=FALSE) +
#   geom_point(aes(color=Diagnosis,alpha=dataset, shape=dataset), size=2) +
#   my_theme() +
#   scale_fill_manual(values=diagnosis_colors) +
#   scale_color_manual(values=c(diagnosis_colors,c('DMSO'='gray40','HU'='orange1','Olaparib'='blue2','SRC7'='orchid1',
#                                                  'DSMO'='gray40','SCR7'='orchid1','BIBR'='green3','ATMi'='brown4'))) +
#   scale_alpha_manual(values=c('FB'=.25,'inhibitors'=1)) +
#   scale_shape_manual(values=c('FB'=16,'inhibitors'=15)) +
#   guides(color=guide_legend(override.aes = list(shape=15,size=2), ncol=3),
#          shape=guide_legend(ncol=1),
#          alpha=guide_legend(ncol=1)) +
#   theme(legend.key.height=unit(1.5,'mm'),
...
features.here <- c('top_clone_occupancy_downsampled',
                   'mean_cluster_size_downsampled',
                   'cluster_gini_downsampled',
                   'mean_length',
                   'donor_complexity',
...
tmp <- cbind(rbind(CH12.data[,features.here],
                   FB.data.pooled[,features.here],
                   QP.data[,features.here],
                   inhibitors.data[,features.here]),
             rbind(CH12.meta %>%
                     dplyr::mutate(Diagnosis=Genotype,
                                   dataset='CH12') %>%
                     dplyr::select(Diagnosis, dataset),
                   human.meta[,c('Diagnosis','dataset')],
                   inhibitors.data[,c('Diagnosis'),drop=FALSE] %>%
                     dplyr::mutate(dataset='inhibitors',
                                   Diagnosis=ifelse(Diagnosis=='ATM','ATMi',Diagnosis)))) %>%
  dplyr::filter(Diagnosis %in% c('WT','Trp53bp1 KO','Brca1 KO','Rif1 KO','Lig4 KO','Control','CVID','TNFRSF13B','ATM','BRCA1',
                                 'DMSO','BIBR','ATMi')) %>%
  tibble::rownames_to_column('sample') %>%
  gather(col,value,features.here) %>%
...
ggplot(tmp, aes(x=ifelse(dataset=='CH12',as.numeric(group),
                         ifelse(dataset=='inhibitors',
                                as.numeric(group) + 2,
                                as.numeric(group) + 1)),
                y=value,fill=group)) +
  geom_boxplot(outlier.shape=NA, lwd=.5) +
  ...
  stat_compare_means(comparisons=list(c(7,8),
                                      ...
                                      c(7,11)),
                     method='wilcox.test',label = "p.signif", hide.ns=TRUE, size=2, vjust=.5) +
  stat_compare_means(comparisons=list(c(13,14),
                                      c(13,15)),
                     method='wilcox.test',label = "p.signif", hide.ns=TRUE, size=2, vjust=.5) +
  facet_wrap(.~col,ncol=4,scales='free_y') +
  my_theme() +
  coord_cartesian(clip='off') +
  scale_fill_manual(values=c(diagnosis_colors,genotype_colors,
                             c('DMSO'='gray40','HU'='orange1','Olaparib'='blue2','SCR7'='orchid1',
                               'BIBR'='green3','ATMi'='brown4','Act'='yellow3'))) +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=.5),
        legend.position='none') +
  scale_x_continuous(breaks=c(seq(1,5), seq(1,5) + 6, seq(1,3) + 12),
                     labels=levels(tmp$group)) +
  labs(x='',y='', color='Diagnosis')
```
