...
meta_clustering <- read.csv('data/cellswitch_meta_clustering.csv', header=1, row.names=1)
...
```{r FigS1X1, fig.width=1.4 * fig_scale, fig.height=1.4 * fig_scale}
vdj_df %>% 
  dplyr::mutate(ncells=factor(gsub('000$','k',ncells), levels=c('50k','100k')),
                Donor=gsub('Donor ','',Donor)) %>%
  dplyr::filter(metric=='cluster_gini_raw') %>%
  dplyr::group_by(ncells, Donor, method) %>%
  dplyr::summarise(mean_gini=mean(value),
                   se_gini=sd(value)/sqrt(length(value))) %>%
  pivot_wider(id_cols=c(ncells,Donor),names_from=method,values_from=c('mean_gini','se_gini')) %>%
  ggplot(aes(x=mean_gini_VDJ, y=mean_gini_SWIBRID,
             xmin=mean_gini_VDJ-se_gini_VDJ,
             xmax=mean_gini_VDJ+se_gini_VDJ,
             ymin=mean_gini_SWIBRID-se_gini_SWIBRID,
             ymax=mean_gini_SWIBRID+se_gini_SWIBRID)) +
  geom_point(aes(color=Donor,shape=ncells),size=1) +
  geom_errorbar(linewidth=.25) +
  geom_errorbarh(linewidth=.25) +
  #scale_x_continuous(breaks=c(.6,.65)) +
  ...
  scale_color_brewer(palette='Set1') +
  theme(strip.background=element_blank()) +
  labs(y='Gini coefficient SWIBRID',x='Gini coefficient VDJ',shape='# cells')
```

```{r FigS1X2, fig.width=2 * fig_scale, fig.height=1.4 * fig_scale}
library(ggpmisc)
PCR.data <- read.csv('data/PCR_data.csv',header=1,row.names=1)
ggplot(PCR.data, aes(x=as.integer(ncycles), y=log10(reads))) + 
  geom_point(aes(color=donor)) + 
  geom_smooth(method='lm', color='black', linewidth=.5) +
  geom_abline(slope=log10(2), intercept=-1.5, linewidth=.5, linetype='dashed') +
  my_theme() + 
  scale_color_brewer(palette='Set2') +
  scale_y_continuous(breaks = log10(c(1, 10, 100, 1000,10000, 100000)),
                     label = c(1, 10, 100, 1000, 10000, 100000)) +
  labs(x='# of cycles',y='# of reads')
```
...
```{r Fig2D, fig.width=1.5 * fig_scale,fig.height=1.75 * fig_scale}
donor_cutoff <- .25
batch_cutoff <- .35
sequencing_cutoff <- .2
...
<!-- the PCR cycles experiment -->

<!-- ```{r PCR_cycles} -->
<!-- PCR.data <- read.csv('../human_samples/20250703_PCR_cycles/output/summary/20250703_PCR_cycles_stats.csv',header=1,row.names=1) %>% -->
<!--   tibble::rownames_to_column('sample') %>% -->
<!--   dplyr::mutate(donor=gsub('[0-9]*_(P[0-9])x([0-9]*)','\\1',sample), -->
<!--                 ncycles=gsub('[0-9]*_(P[0-9])x([0-9]*)','\\2',sample))  -->

<!-- PCR.X <- PCR.data[,c('sample',selected.cols)] %>% -->
<!--   dplyr::mutate_if(is.numeric,scale) %>% -->
<!--   tibble::column_to_rownames('sample') -->

<!-- PCR.meta <- PCR.data[,c('ncycles','donor')] -->

<!-- cols.here <- c('reads','clusters_raw','PCR_bias_length','PCR_bias_GC') -->
<!-- PCR.data %>% -->
<!--   gather(variable, value, cols.here) %>% -->
<!--   dplyr::select(variable, value, ncycles, donor) %>% -->
<!--   ggplot(aes(x=ncycles, y=value, fill=ncycles)) +  -->
<!--   geom_boxplot(position=position_dodge(width=.75), width=.7, outlier.shape=NA, lwd=.25) +  -->
<!--   geom_point(aes(color=donor), position=position_jitterdodge(jitter.width=.2, dodge.width=.75, jitter.height = 0), size=.5) +  -->
<!--   my_theme() + -->
<!--   coord_cartesian(clip='off') + -->
...
<!-- take <- (rowSums(is.na(PCR.X))==0) &  (PCR.meta$ncycles %in% c('15','20','25')) -->
<!-- pca <- prcomp(PCR.X[take,]) -->
<!-- pcadat <- cbind(pca$x,PCR.meta[take,]) -->

<!-- ggplot(pcadat,aes(x=PC1,y=PC2)) + -->
<!--   geom_point(aes(color=donor, shape=ncycles),size=3) + -->
<!--   my_theme() + -->
...
<!-- cellswitch.data <- read.csv('../human_samples/20240307_hg38_results/output/summary/20240307_human_samples_stats.csv', -->
...
<!--                                              '21085'='B', -->
<!--                                              '21086'='C'))) %>% -->
<!--   dplyr::select(Donor,ncells,Batch,sample,selected.cols) -->


<!-- cellswitch.X <- cellswitch.data[,c('sample',selected.cols)] %>% -->
<!--   dplyr::mutate_if(is.numeric,scale) %>% -->
<!--   tibble::column_to_rownames('sample') -->

<!-- cellswitch.meta <- cellswitch.data[,c('Donor','ncells')] -->

<!-- take <- (rowSums(is.na(cellswitch.X))==0) #& (cellswitch.meta$ncells > 10000) -->
<!-- pca <- prcomp(cellswitch.X[take,]) -->
<!-- pcadat <- cbind(pca$x,cellswitch.meta[take,]) -->

<!-- ggplot(pcadat,aes(x=PC1,y=PC2)) + -->
<!--   geom_point(aes(fill=ncells),size=3,pch=21) + -->
...
<!--   scale_fill_distiller(palette='Spectral',limits=c(100,200000), trans='log', breaks=c(1000,10000,100000)) + -->
<!--   labs(x=paste0('PC 1 (',100*signif(pca$sdev[1]**2/sum(pca$sdev**2),3),'%)'), -->
<!--        y=paste0('PC 2 (',100*signif(pca$sdev[2]**2/sum(pca$sdev**2),3),'%)'), -->
<!--        color='Donor',fill='# cells') -->
...
<!-- ggplot(pcadat,aes(x=PC1,y=PC2)) + -->
<!--   geom_point(aes(fill=ncells,color=Donor),size=3,pch=21) + -->
<!--   geom_line(aes(group=Donor,color=Donor)) + -->
<!--   my_theme() + -->
<!--   theme(aspect.ratio=pca$sdev[2]**2/pca$sdev[1]**2, -->
<!--         legend.key.size=unit(1.5,'mm'), -->
...
train_batches <- c('20220`4`25','20221118','20221212','20230120','20230213')
...
FB.var %>%  dplyr::left_join(columns, by='col') %>%
  ...
  #geom_jitter(aes(color=ifelse(col %in% selected.cols, 'selected','unused')), position=position_jitter(width=.25), size=.25) +
  #scale_color_manual(values=c('unused'='gray','selected'='black')) +
  my_theme() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1),
        strip.clip='off',
        legend.position='none',
...
```{r FigS4H, fig.width=2.5 * fig_scale, fig.height=1.5 * fig_scale}
...
show top examples for features related to Treatment or IV

```{r FigS4X1, fig.width=1.5 * fig_scale, fig.height=1.5 * fig_scale}
FB.data.pooled %>%
  dplyr::mutate(IV=factor(IV,levels=c('none','ivig','scIg'))) %>%
  ggplot(aes(x=IV, y=cluster_entropy)) + 
  geom_boxplot(outlier.shape=NA, lwd=.5) +
  geom_jitter(aes(color=status), width=.1, size=.5) + 
  stat_compare_means(comparisons=list(c('none','ivig'), c('none','scIg'),c('ivig','scIg')),
                     method='wilcox.test',label = "p.signif", hide.ns=TRUE, size=2.5) +
  my_theme() + 
  coord_cartesian(clip='off') +
  scale_color_manual(values=diagnosis_colors) +
...
FB.data.pooled %>%
  dplyr::mutate(Treatment=factor(Treatment,levels=c('none','cortison','infliximab','prednison','sirolimus','steroid','trixomazl'))) %>%
  ggplot(aes(x=Treatment, y=occupancy_top_cluster)) + 
  geom_boxplot(outlier.shape=NA, lwd=.5) +
  geom_jitter(aes(color=status), width=.1, size=.5) + 
  my_theme() + 
  coord_cartesian(clip='off') +
